...
type AppError struct {
	...
	Status  int
	Message string
	// Details carries structured, client-safe context such as the per-conflict
	// reasons that invalidate a batch submission.
...
// conflictDetails reports a 409 with structured per-resource reasons.
func conflictDetails(message string, details map[string]any, err error) error {
	return &AppError{Code: CodeConflict, Status: http.StatusConflict, Message: message, Details: details, Err: err}
}
